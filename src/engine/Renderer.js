...
export class Renderer {
    constructor(canvas, cityModel) {
        ...
        // Storage for overlay data
        this.routePath = [];
        this.servicePoints = []; // {x,y,type}
        this.safetyZones = [];
    ...
    /**
     * Set the current route path to display
     * @param {Array} path
     */
    setRoutePath(path) {
        this.routePath = path || [];
    ...
    /**
     * Provide service point data
     * @param {Array} points - array of {x,y,type}
     */
    ...
    render() {
        ...
        if (this.layers.route && this.routePath && this.routePath.length) {
            this.drawRoutePath(cellSize);
        }

        // Draw service points
        if (this.layers.services && this.servicePoints && this.servicePoints.length) {
            this.drawServicePoints(cellSize);
        }
    ...
    drawZones(cellSize) {
        ...
        for (let y = 0; y < this.city.gridSize; y++) {
            for (let x = 0; x < this.city.gridSize; x++) {
                ...
                // Draw ramp indicator if accessibility.ramp is true
                if (zone.accessibility && zone.accessibility.ramp) {
                    ctx.fillStyle = '#ffffff';
                    const radius = cellSize * 0.15;
                    ctx.beginPath();
                    ctx.arc(screenX + radius + 2, screenY + radius + 2, radius, 0, Math.PI * 2);
                    ctx.fill();
                }
            }
        }
    ...
    /**
     * Draw a route path if provided
     */
    drawRoutePath(cellSize) {
        const ctx = this.ctx;
        ctx.fillStyle = 'rgba(255,0,255,0.4)';
        this.routePath.forEach(pt => {
            const { x, y } = this.gridToScreen(pt.x, pt.y);
            ctx.fillRect(x + 1, y + 1, cellSize - 2, cellSize - 2);
        });
    }

    /**
     * Draw simple service markers (color-coded dots)
     */
    drawServicePoints(cellSize) {
        const ctx = this.ctx;
        this.servicePoints.forEach(pt => {
            const { x, y } = this.gridToScreen(pt.x, pt.y);
            switch (pt.type) {
                case 'hospital': ctx.fillStyle = '#e53e3e'; break;
                case 'school': ctx.fillStyle = '#3182ce'; break;
                case 'police': ctx.fillStyle = '#22543d'; break;
                case 'park': ctx.fillStyle = '#38a169'; break;
