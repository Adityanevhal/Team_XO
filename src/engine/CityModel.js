...
export class CityModel {
    constructor(size = 'medium') {
        ...
        // Initialize grid with empty zones, accessibility flags and service markers
        ...
        // Service locations for quick lookup
        ...
        // Accessibility features count (for stats)
    ...
    /**
     * Set an accessibility feature on a grid cell
     * @param {number} x
     * @param {number} y
     * @param {string} feature - 'ramp'|'elevator'|'sidewalkWidth'
     * @param {any} value
     */
    setAccessibility(x, y, feature, value) {
        const cell = this.getZone(x, y);
        if (!cell) return false;
        if (!cell.accessibility) cell.accessibility = {};
        cell.accessibility[feature] = value;

        if (feature === 'ramp') {
            this.accessibilityStats.ramps += value ? 1 : -1;
        }
        if (feature === 'elevator') {
            this.accessibilityStats.elevators += value ? 1 : -1;
        }
        return true;
    }

    /**
     * Mark a service at a cell (hospital, school, etc.)
     * @param {number} x
     * @param {number} y
     * @param {string} type
     */
    addService(x, y, type) {
        const cell = this.getZone(x, y);
        if (!cell) return false;
        if (!cell.services) cell.services = {};
        cell.services[type] = true;

        if (this.serviceLocations[type]) {
            this.serviceLocations[type].push({ x, y });
        }
        return true;
    }

    /**
     * Find nearest service location of a given type
     * @param {number} x
     * @param {number} y
     * @param {string} type
     * @returns {{x:number,y:number}|null}
     */
    findNearestService(x, y, type) {
        const list = this.serviceLocations[type] || [];
    ...
    clear() {
        ...
        // reset service lookups and accessibility stats
        this.serviceLocations = {
            hospital: [],
            school: [],
            police: [],
