...
class SmartCityApp {
    constructor() {
        ...
        this.router = new Router(this.city);
    ...
    initUI() {
        ...
        // Voice command
        document.getElementById('btn-voice').addEventListener('click', () => this.toggleVoiceRecognition());

        // Route finding
        document.getElementById('btn-find-route').addEventListener('click', () => this.findRoute());

        // Service planner
        document.getElementById('btn-plan-services').addEventListener('click', () => this.planServices());

        // Citizen feedback
        document.getElementById('btn-report-issue').addEventListener('click', () => this.reportIssue());

        // Sustainability advisor
        document.getElementById('btn-get-sustainability').addEventListener('click', () => this.getSustainabilityAdvice());

        // Safety analysis
        document.getElementById('btn-analyze-safety').addEventListener('click', () => this.analyzeSafety());
    ...
    async generateCity() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // clear previous route and service overlays
                this.renderer.clearRoute();
                this.renderer.setServicePoints([]);
                this.renderer.render();
    ...
    draw(e) {
        const pos = this.renderer.screenToGrid(e.offsetX, e.offsetY);
        if (this.currentTool === 'access') {
            const cell = this.city.getZone(pos.x, pos.y);
            if (cell) {
                const hasRamp = cell.accessibility?.ramp;
                this.city.setAccessibility(pos.x, pos.y, 'ramp', !hasRamp);
    ...
    async findRoute() {
        ...
        if (!start || !end) {
            showToast('Invalid start/end coordinates', 'warning');
            return;
        }
        const path = this.router.findPath(start, end, { accessible });
        if (path && path.length) {
            this.renderer.setRoutePath(path);
            document.getElementById('route-result').textContent = `Path length ${path.length}`;
        } else {
    ...
    async planServices() {
        const pop = parseInt(document.getElementById('service-population').value, 10) || 0;
        const area = parseFloat(document.getElementById('service-area').value) || 0;
        const result = await this.aiGenerator.planServices(pop, area);
        const points = [];
        Object.keys(result).forEach(type => {
            result[type].forEach(pt => {
                if (pt && typeof pt.x === 'number') {
                    this.city.addService(pt.x, pt.y, type);
                    points.push({ x: pt.x, y: pt.y, type });
                }
            });
        });
        this.renderer.setServicePoints(points);
        document.getElementById('service-result').textContent = 'Services placed';
    }
    ...
    reportIssue() {
        ...
        this.feedbackManager.add(type, desc);
        document.getElementById('feedback-status').textContent = 'Issue reported';
        // Optionally classify
        this.aiGenerator.classifyFeedback(this.feedbackManager.getAll()).then(classified => {
            console.log('Feedback classified', classified);
        });
    }

    async getSustainabilityAdvice() {
        const prompt = document.getElementById('ai-prompt').value.trim();
        const suggestions = await this.aiGenerator.suggestGreen(prompt);
        const container = document.getElementById('sustainability-result');
    ...
    async analyzeSafety() {
        const prompt = document.getElementById('ai-prompt').value.trim();
        const data = await this.aiGenerator.analyzeSafety(prompt);
        const container = document.getElementById('safety-result');
        if (Array.isArray(data)) {
            container.innerHTML = data.map(s => `<div>${s.suggestion || ''} @ (${s.x},${s.y})</div>`).join('');
            // mark on map
            const pts = data.map(s => ({ x: s.x, y: s.y, type: 'safety' }));
            this.renderer.setServicePoints(pts);
        } else {
    ...
    handleVoiceCommand(text) {
        ...
        if (lower.includes('generate city')) {
            document.getElementById('ai-prompt').value = text;
            this.generateCity();
        } else if (lower.includes('accessible route')) {
            showToast('Please specify start/end coordinates manually for now', 'warning');
        }
    }
